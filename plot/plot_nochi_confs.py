...
ts_align, confs_align, confs_err_align = utils.t_confs(dirname_align)
# ts_reflect, confs_reflect, confs_err_reflect = utils.t_confs(dirname_reflect)

# ax.errorbar(ts_align, confs_align, yerr=confs_err_align,
#             label='Aligning', c=ejm_rcparams.set2[0])
# ax.errorbar(ts_reflect, confs_reflect, yerr=confs_err_reflect,
#             label='Reflecting', c=ejm_rcparams.set2[1])
# ax.axhline(color=ejm_rcparams.almost_black)

# ax.legend(loc='upper left', fontsize=26)
# ax.set_xlabel(r'$t / \mathrm{s}$', fontsize=35)
# ax.set_ylabel(r'$\xi$', fontsize=35)
# ax.tick_params(axis='both', labelsize=26, pad=10.0)
# ax.set_xlim(0.0, None)
# # ax.set_ylim(-0.01, 1.01)

# if save_flag:
#     plt.savefig('plots/nochi_t_confs.pdf', bbox_inches='tight')
# else:
#     plt.show()
